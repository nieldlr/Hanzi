...
var decompose = function(character, typeOfDecomposition) {
  ...
  if (typeOfDecomposition == null) {
    object = {
      ...
      components4: primitiveDecomposition(character)
    };
    ...
    object = {
      character: character,
      components: primitiveDecomposition(character)
    };
  } else {
    return;
  }
  ...
  /*
   * Primitive decomposition (基础部件): break a character into its recognisable
   * building blocks, derived purely from the composition structure. Stop at a
   * node when isStop() says it is a fused/contained/modified unit; otherwise keep
   * splitting. Independent single strokes are kept (旦 -> 日, 一), but unnamed
   * numbered grouping nodes are recursed through, not emitted. The result stops at
   * meaningful units (成 -> 万, 戈) instead of over-splitting into the strokes that
   * radicalDecomposition gives.
   */
  function primitiveDecomposition(character) {
    if (isPrimitive(character)) {
      return [character];
    }
    var components = getComponents(character);
    if (!Array.isArray(components)) {
      // an unrecognised / atomic token that is not a primitive — echo a real
      // glyph back, but drop a bare unnamed numbered grouping node
      return isNaN(character) ? [character] : [];
    }
    // "split" configuration (衣 chars: 哀 = split(衣,口)): the first component is
    // the frame that the second splits apart. Keep the frame whole (never
    // decompose it) and decompose only the filling — 哀 -> [衣, 口], 襄 -> [衣, …].
    if (
      ifComponentExists(character) &&
      characters[character].typeOfDecomposition.split('/')[0] === 'split'
    ) {
      var split_array = [components[0]];
      for (var s = 1; s < components.length; s++) {
        split_array = split_array.concat(primitiveDecomposition(components[s]));
      }
      return split_array;
    }
    var final_array = [];
    for (var j = 0; j < components.length; j++) {
      ...
      if (!isNaN(component) && isUnnameableBlob(component)) {
        // a numbered grouping node built only from strokes and trivial stacks
        // (发's top-left, 隹's right, 师's left) is a coherent but glyph-less
        // component — surface it as one "No glyph" gap rather than loose strokes.
        // isUnnameableBlob (not resolvesToStrokesOnly) so a 二 inside still
        // dissolves instead of forcing the node to shred
        final_array.push(noglyph);
      } else {
        // recurse: a real component, or a numbered node that still holds a
        // named component to expose (e.g. the 爫冖 grouping in 爱)
        final_array = final_array.concat(primitiveDecomposition(component));
      }
    }
    return final_array;
  }
